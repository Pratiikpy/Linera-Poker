...
################################################################################
# Linera Poker - Conway Testnet Deployment (Docker Version)
# Deploys contracts to Conway testnet using Docker container
################################################################################
...
# Wallet paths (Conway-specific)
export LINERA_WALLET="/tmp/linera-conway/wallet.json"
export LINERA_KEYSTORE="/tmp/linera-conway/keystore.json"
export LINERA_STORAGE="rocksdb:/tmp/linera-conway/client.db"
...
log_info "Step 1/6: Checking WASM contracts..."
...
mkdir -p "/tmp/linera-conway"

# Remove existing wallet if present (fresh start for Conway)
if [ -f "${LINERA_WALLET}" ]; then
    log_warning "Removing existing Conway wallet for fresh deployment..."
    rm -rf "/tmp/linera-conway"
    mkdir -p "/tmp/linera-conway"
fi

# Initialize wallet with Conway faucet
log_info "Connecting to Conway faucet: ${FAUCET_URL}"
linera wallet init --faucet "${FAUCET_URL}"
log_success "Wallet initialized with Conway testnet!"

################################################################################
# Step 3: Request Chains from Conway Faucet
################################################################################
...
TABLE_APP_ID=$(linera publish-and-create \
    "${TABLE_CONTRACT}" "${WASM_DIR}/table_service.wasm" \
...
PLAYER_A_HAND_APP_ID=$(linera publish-and-create \
    "${HAND_CONTRACT}" "${WASM_DIR}/hand_service.wasm" \
...
echo "├────────────────────────────────────────────────────────────────┤"
echo "│  Network:        Conway Testnet (PUBLIC)                      │"
...
################################################################################
# Save deployment info to file for host access
################################################################################

cat > /tmp/deployment-info.txt << EOF
TABLE_CHAIN_ID=${TABLE_CHAIN_ID}
PLAYER_A_CHAIN_ID=${PLAYER_A_CHAIN_ID}
PLAYER_B_CHAIN_ID=${PLAYER_B_CHAIN_ID}
TABLE_APP_ID=${TABLE_APP_ID}
PLAYER_A_HAND_APP_ID=${PLAYER_A_HAND_APP_ID}
PLAYER_B_HAND_APP_ID=${PLAYER_B_HAND_APP_ID}
FAUCET_URL=${FAUCET_URL}
SERVICE_URL=${SERVICE_URL}
EOF

log_success "Deployment complete! Configuration saved to /tmp/deployment-info.txt"
log_info "Next: Build frontend and deploy to Netlify"
echo ""
